...
namespace Stemkit.Auth.Services.Implementation
{
    public class RefreshTokenService : IRefreshTokenService
    {
        ...
        public async Task<AuthResponse> RefreshTokensAsync(string token, string ipAddress)
        {
            if (string.IsNullOrWhiteSpace(token))
            {
            ...
            {
                _logger.LogWarning("Refresh token has been revoked: {Token}", token);
                return new AuthResponse { Success = false, Message = "Revoked refresh token." };
            }
            ...
            var newAccessToken = _jwtTokenService.GenerateJwtToken(user.UserId, roleNames);
