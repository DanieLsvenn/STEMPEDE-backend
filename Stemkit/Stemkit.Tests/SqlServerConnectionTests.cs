...
namespace Stemkit.Tests
{
    ...
    {
        private const string ConnectionString = "Server=DESKTOP-USDRF6H\\SQLEXPRESS;Database=STEMKITshop;User ID=sa;Password=p@ssw0rd12345;TrustServerCertificate=True;";
        ...
        public async Task Test_SqlServerConnection_ShouldConnectToDatabase()
        {
            ...
            try
            {
                ...
                using (var command = new SqlCommand("SELECT DB_NAME()", connection))
                {
                    ...
                    Assert.Equal("STEMKITshop", databaseName.ToString());
                }
            }
            catch (Exception ex)
            {
